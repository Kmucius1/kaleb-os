...
import AtlasContext from '@/components/rhythm/AtlasContext'

type Msg = { role: 'user' | 'assistant'; content: string; actions?: { tool: string }[] }

const QUICK = [
  { label: 'Daily Briefing', sub: 'What today asks of you', icon: FileText, color: '#8b5cf6', prompt: 'Give me my daily briefing for today.' },
  { label: 'Rebalance My Day', sub: 'Something changed', icon: Activity, color: '#a855f7', prompt: 'Something ran long — rebalance the rest of my day and show me exactly what moves and why.' },
  { label: 'Capture a Thought', sub: 'Talk, I’ll file it', icon: Mic, color: '#60a5fa', prompt: 'VOICE' },
  { label: 'Ask Anything', sub: 'Questions, strategy', icon: MessageCircle, color: '#6366f1', prompt: '' },
]

const SUGGESTIONS = [
  { text: 'Analyze today — am I living by my values?', icon: Activity, color: '#a855f7' },
  { text: 'What patterns are showing up in my journals?', icon: Lightbulb, color: '#8b5cf6' },
  { text: 'Review my trading discipline this week', icon: TrendingUp, color: '#fbbf24' },
  { text: 'When should I take my Horizon Walk today?', icon: Rocket, color: '#a78bfa' },
  { text: 'How is DRYP tracking this month?', icon: Target, color: '#fb923c' },
  { text: 'Where is my fitness and sleep trending?', icon: Rocket, color: '#34d399' },
]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', maxWidth: 780, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <Link href="/" title="Home" style={{ ...iconBtn(false), textDecoration: 'none' }}><LayoutDashboard size={16} /></Link>
        </div>
      </div>

      {/* Where Kaleb is in his day — Atlas knows before he asks. */}
      <AtlasContext />
