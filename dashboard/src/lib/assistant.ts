...
export const TOOLS = [
  ...
  { type: "function", function: { name: "add_task", description: "Add a to-do/reminder/follow-up. Always set owner and priority — an untriaged task is invisible in a list this size.", parameters: { type: "object", properties: { title: { type: "string" }, deadline: { type: "string" }, owner: { type: "string", enum: ["kaleb", "team", "other"], description: "kaleb = his own commitment. team = he owns the outcome, someone else does it. other = another party's action item overheard in a meeting (the banker's steps, a client's internal paperwork, logistics someone else runs) — use this freely, it keeps his list clean." }, priority: { type: "number", description: "1-10. 9-10 money/deadline/blocking others. 7-8 revenue or an active blocker. 4-6 normal work. 1-3 trivia, vague restatements, someday." }, area: { type: "string", enum: ["dryp", "ehm", "linkdup", "kaleb-os", "trading", "commerce", "clients", "personal", "admin", "other"] } }, required: ["title", "owner", "priority"] } } },
...
async function runTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case "add_task": return await addTask({
      title: String(args.title), deadline: args.deadline as string,
      owner: args.owner as string, priority: args.priority as number,
      area: args.area as string, source: args.source as string,
    });
